 ...
 *
 * It would be nicer to pass the struct quic_dgram instead of having all these
 * parameters, but that would cause unnecessary copies of the source and
 * destination addresses which are quite large (struct sockaddr_storage).
 */
                            ...
                            unsigned int cid_tid)
{
	...
	buf = bring_write_reserve(dst, bring_len);
	if (!buf)
		return 0;
	...
	bring_write_commit(dst, buf, bring_len);
                                     ...
                                     struct sockaddr_storage *daddr)
{
	...
	if (!quic_dgram_write(pos, len, owner, dcid, dcid_len, saddr, daddr, cid_tid))
		goto err;
